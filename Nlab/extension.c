...
void B_sub(stacktype* a, stacktype* b, stacktype* c);
void B_div(stacktype* a, stacktype* b, stacktype* c);
void B_xor(stacktype* a, stacktype* b, stacktype* c);
void B_pow(stacktype* a, stacktype* b, stacktype* c);
...
bool isnum(prog* p)
{
    if(((p->wds[p->cw][0]) == '-') || (((p->wds[p->cw][0]) <= 57) && ((p->wds[p->cw][0]) >= 48))){
        ;
    }else{
        return false;
    }
...
void Unaryop(prog* p, stack* line_stack)
{
    ...
    p->cw = p->cw - 1;
    if(isnum(p)){
        perror("There should be no number before unary operation\n");
        exit(EXIT_FAILURE);
    }
    p->cw = p->cw + 1;
#ifdef INTERP
    ...
    if(strcmp(p->wds[p->cw], "U-NOT") == 0){
        ...
        p->cw = p->cw + 1;
        return;
    }else if(strcmp(p->wds[p->cw], "U-SGN") == 0){
#ifdef INTERP
        stacktype* x = (stacktype*)ncalloc(1, sizeof(stacktype));
        x->rows = c->rows;
        x->cols = c->cols;
        ...
        p->cw = p->cw + 1;
        return;
    }else if(strcmp(p->wds[p->cw], "U-MAX") == 0){
#ifdef INTERP
        stacktype* x = (stacktype*)ncalloc(1, sizeof(stacktype));
        x->rows = 1;
        x->cols = 1;
        ...
        p->cw = p->cw + 1;
        return;
    }else if(strcmp(p->wds[p->cw], "U-MIN") == 0){
#ifdef INTERP
        stacktype* x = (stacktype*)ncalloc(1, sizeof(stacktype));
        x->rows = 1;
        x->cols = 1;
        ...
        perror("UNARYOP should be followed by U-NOT ,U-EIGHTCOUNT or U-SGN.\n");
        exit(EXIT_FAILURE);
    }
}
...
void Binaryop(prog* p, stack* line_stack)
{
    ...
    if(strcmp(p->wds[p->cw], "B-AND") == 0){
        ...
        p->cw = p->cw + 1;
        return;
    }else if(strcmp(p->wds[p->cw], "B-SUB") == 0){
#ifdef INTERP
        if(exe_Pop(line_stack, a, b)){
            if(issamesize(a, b)){
                B_sub(a, b, c);
        ...
        p->cw = p->cw + 1;
        return;
    }else if(strcmp(p->wds[p->cw], "B-DIV") == 0){
#ifdef INTERP
        if(exe_Pop(line_stack, a, b)){
            if(issamesize(a, b)){
                B_div(a,b,c);
        ...
        p->cw = p->cw + 1;
        return;
    }else if(strcmp(p->wds[p->cw], "B-XOR") == 0){
#ifdef INTERP
        if(exe_Pop(line_stack, a, b)){
            if(issamesize(a, b)){
                B_xor(a,b,c);
        ...
        p->cw = p->cw + 1;
        return;
    }else if(strcmp(p->wds[p->cw], "B-POW") == 0){
#ifdef INTERP
        if(exe_Pop(line_stack, a, b)){
            if(issamesize(a, b)){
                B_pow(a,b,c);
...
void B_pow(stacktype* a, stacktype* b, stacktype* c)
{
    if(a->rows == 1 && a->cols == 1){
        ...
        for(int j=0; j<c->rows; j++){
            for(int i=0; i<c->cols; i++){
                c->arr[j][i] = pow(b->arr[j][i], a->arr[0][0]);
            }
        }
    }else if(b->rows == 1 && b->cols == 1 && a->rows != b->rows && a->cols != b->cols){
        perror("the first value must be the same size of the second\n");
        exit(EXIT_FAILURE);
    }else{
        c->rows = a->rows;
        c->cols = a->cols;
        for(int j=0; j<c->rows; j++){
            for(int i=0; i<c->cols; i++){
                for(int m=0; m<a->arr[j][i]; m++){
                    c->arr[j][i] = pow(b->arr[j][i], a->arr[j][i]);
                }
            }
        }
    }
